        ...
        register_sidebar(array('name' => 'Footer 4 Column',
            ...
            'after_title' => '</h2><div class="sc_small_line widget_line"><span class="small_l_left"></span></div>',
				...
				register_sidebar(array(
					'name' => 'WPL Slider',
					'id' => 'wpl_slider',
					'description' => 'Appears in the home page',
...
function ref_redirect(){
 if( isset($_GET['ref']) && preg_match('~\/\?ref=\d+~', $_SERVER['REQUEST_URI']) ){
  global $wpdb;
  $mls_id = (int)$_GET['ref'];
  $id = $wpdb->get_var( "SELECT id FROM `". $wpdb->prefix ."wpl_properties` WHERE mls_id = ".$mls_id );
  if( $id ){
   $url = '/woning/'.$id.'-';
...
add_action('viki_single_product_header','single_product_header',10);

function single_product_header() {
    wc_get_template( 'single-product/viki/single-product-header.php' );
}


add_action('viki_single_product_cart_options','option_design',10);

function option_design() {
    wc_get_template( 'single-product/viki/single-product-option-design.php' );
}

add_action('viki_single_product_cart_options','option_type',10);
function option_type() {
    wc_get_template( 'single-product/viki/single-product-option-type.php' );
}


add_action('viki_single_product_cart_options','option_material',10);
function option_material() {
    wc_get_template( 'single-product/viki/single-product-option-material.php' );
}

add_action('viki_single_product_cart_options','option_sides',10);
function option_sides() {
    wc_get_template( 'single-product/viki/single-product-option-sides.php' );
}

add_action('viki_single_product_cart_options_price','option_price',10);
function option_price() {
    wc_get_template( 'single-product/viki/single-product-option-price.php' );
}


remove_action('woocommerce_composite_component_selections_progressive','wc_cp_add_sorting',10);
add_image_size('component_type_image',130,165);
