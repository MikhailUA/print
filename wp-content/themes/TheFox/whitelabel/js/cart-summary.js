...
$(document).ready(function()
{
	...
	$('.gift_message').on('change', function(){updateGiftMessage($(this).val());});
...
function changeAddressDelivery(obj, line)
{
		...
		$.ajax({
			...
			success: function(jsonData)
			{
				...
				else
				{
					...
					if (window.ajaxCart != undefined)
						ajaxCart.refresh();
					updateAddressId(id_product, id_product_attribute, old_id_address_delivery, new_id_address_delivery, line);
					cleanSelectAddressDelivery();
				}
			}
		});
...
function updateAddressId(id_product, id_product_attribute, old_id_address_delivery, id_address_delivery, line)
{
	...
	clicked_line = line;
	clicked_line.closest('ul').find('li a').each(function() {
		$(this).attr('id', $(this).attr('id').replace(/_\d+$/, '_' + id_address_delivery)).removeClass('address_' + old_id_address_delivery).addClass('address_' + id_address_delivery).find('span[id^=cart_quantity_custom_], span[id^=total_product_price_], input[name^=quantity_], .cart_quantity_down, .cart_quantity_up, .cart_quantity_delete').each(function(){
				/*if (typeof($(this).attr('name')) != 'undefined')
					$(this).attr('name', $(this).attr('name').replace(/_\d+(_hidden|)$/, '_' + id_address_delivery));
...
function deleteProductFromSummary(id)
{
  ...
  if (typeof(ids[4]) != 'undefined')
	key_id = parseInt(ids[4]);
  //if (typeof(ids[5]) != 'undefined')
	//id = id.replace('_'+new_cart_id,'');
  //alert(id);

	// get nid
	new_cart_id = $('a#'+id).attr('nid');
	...
	$.ajax({
		...
		success: function(jsonData)
		{
			...
			else
			{
				...
				{
					 div_id=key_id+'_'+ids[0]+'_'+ids[1]+'_'+ids[2]+'_'+ids[3];
					$('#product'+div_id).fadeOut('slow', function() {
						$(this).remove();
						cleanSelectAddressDelivery();
						if (!customizationId)
							refreshOddRow();
					  ...
					  $('#product_options'+div_id).fadeOut('slow', function() {
						$(this).remove();
						cleanSelectAddressDelivery();
						if (!customizationId)
							refreshOddRow();
	...
	$.ajax({
		...
		success: function(jsonData)
		{
			...
			else
			{
				...
				//update UI
				duplicateProductonUI(id, $(jsonData.summary.products).last(), (jsonData.summary.products.length +100) );
			}
		},
		error: function(XMLHttpRequest, textStatus, errorThrown) {
	/*		if (textStatus !== 'abort')
...
function duplicateProductonUI(old, newProd, newNum)
{
	var new_cart_id = newProd[0]['new_cart_id'];
	var new_prod_class = 'class_'+new_cart_id;
	var old_ids = old.split('_');
	var old_prod_id = 'product'+old_ids[4] +'_'+old_ids[0]+'_'+old_ids[1]+'_'+old_ids[2]+'_'+old_ids[3];
	var new_prod_id = 'product'+newNum +'_'+old_ids[0]+'_'+old_ids[1]+'_'+old_ids[2]+'_'+old_ids[3];

	var new_manage_id = old_ids[0]+'_'+old_ids[1]+'_'+old_ids[2]+'_'+old_ids[3]+ '_'+newNum;

	var old_product_html = $('#'+old_prod_id).clone();

	// set new id
	$(old_product_html).attr('id', new_prod_id);
	// set new class
	$(old_product_html).removeClass().addClass(new_prod_class);


	//change old nid and id in manage section
	$(old_product_html).find('#'+old).each(function(){

		$(this).attr('nid', new_cart_id);
		$(this).attr('id', new_manage_id);

	 })

	// bind clicks
	$(old_product_html).find('.cart_quantity_up').on('click', function(){upQuantity(new_manage_id); return false;});
	$(old_product_html).find('.cart_quantity_delete').on('click', function(){deleteProductFromSummary(new_manage_id); return false;});

	// prepend product
	$(old_product_html).prependTo($('#'+old_prod_id).parent());
...
function updateCartSummary(json)
{
	...
	for (i in product_list)
	{
		...
		initial_price = '';
		if (typeof(product_list[i].price_without_quantity_discount) !== 'undefined' && product_list[i].price_without_quantity_discount !== null)
	...
	if(json.total_cart_file_check !== 'undefined'){
		$('#total_cart_file_check').html(formatCurrency(json.total_cart_file_check, currencyFormat, currencySign, currencyBlank));
	}
	...
	if (json.free_ship > 0)
	{
		$('.show-when-shipping-is-free').show();
		$('.show-when-shipping-is-not-free').hide();
	}
	else
	{
		$('.show-when-shipping-is-not-free').show();
		$('.show-when-shipping-is-free').hide();
	}
	...
	if(json.services_level_price_tax_excl) {
		$('#total_service_levels_price').html(formatCurrency(json.services_level_price_tax_excl, currencyFormat, currencySign, currencyBlank))
	}
...
$(document).ready(function() {
	// currencySign = $.trim($('#total_price').text())[0];
...
function updateGiftMessage(message)
{
	$.ajax({
		...
		data: 'controller=cart&ajax=true'
			...
			+'&allow_refresh=1',
		success: function(jsonData){
			$('#reference-content').text(message);
		}
	});
}
